function EMD_decreasing(NAI)

    % This code implements Algorithm 7 in Section EC.8 with decreasing gamma.
    ...
    % This code aims to obtain the two-sided bound with improved lower
    % bounds. 
    ...
    for r2 = 1:10
        ...
        while iter <= iter_max % entropic mirror descent starts
            

            gamma = 5/sqrt(1 + iter/1000); % decreasing learning rate
            ...
            numer = numer - gamma * sum(grad_old.*(alpha_old - alpha_new)) - sum(alpha_new.*log(alpha_new./alpha_old)); % numerator of the upper bound
            denom = denom + gamma; % denominator of the upper bound
        ...
        if i == 1
            save(strcat('EMD_macro_data/Baseline_decreasing/macro_', num2str((AI-1)*10+r2), '.mat'), 'LDRtilde_at_alpha_opt', 'alpha_opt_sol', 'iter_size', 'LDRtilde_upper', 'gap_upper_bound');
        elseif i == 2
            save(strcat('EMD_macro_data/Scenario1_decreasing/macro_', num2str((AI-1)*10+r2), '.mat'), 'LDRtilde_at_alpha_opt', 'alpha_opt_sol', 'iter_size', 'LDRtilde_upper', 'gap_upper_bound');
        end
    end
