function EMD_constant_gamma(NAI, gamma)
    ...
    % We made this code to address the reviewer's concern on the
    % sensitivity to the bounds in the selection of constant gamma. 
    % We set relatively small iteration numbers (5x10^5) in this code.
    ...
    switch i
        case 1
            ...
            iter_max = 5 * 10^5; % maximum iteration 
            save_folder_name = strcat('EMD_macro_data/Baseline_constant_gamma/gamma_', num2str(gamma)); 
        ...
        case 2
            ...
            iter_max = 5 * 10^5; % maximum iteration 
            save_folder_name = strcat('EMD_macro_data/Scenario1_constant_gamma/gamma_', num2str(gamma));
    end

    if ~isfolder(save_folder_name)
    ...
    for r2 = 1:10
        ...
        if i == 1
            save(strcat(save_folder_name, '/macro_', num2str((AI-1)*10+r2), '.mat'), 'LDRtilde_at_alpha_opt', 'alpha_opt_sol', 'iter_size', 'LDRtilde_upper', 'gap_upper_bound');
        elseif i == 2
            save(strcat(save_folder_name, '/macro_', num2str((AI-1)*10+r2), '.mat'), 'LDRtilde_at_alpha_opt', 'alpha_opt_sol', 'iter_size', 'LDRtilde_upper', 'gap_upper_bound');
        end
    end
