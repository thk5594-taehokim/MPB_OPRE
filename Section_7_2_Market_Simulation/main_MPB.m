function main_MPB(AI)
    ...
    for r = 1 : macrorep
        ...
        [PCS_path, FNR_path, ACC_path, N] = Allocation_EA(T_vec, par);
        stat_EA = update_stat(stat_EA, macrorep, PCS_path, FNR_path, ACC_path, N);

        [PCS_path, FNR_path, ACC_path, N] = Allocation_BM(T_vec, par, "COCBA");
        stat_COCBA = update_stat(stat_COCBA, macrorep, PCS_path, FNR_path, ACC_path,N);

        [PCS_path, FNR_path, ACC_path,N] = Allocation_BM(T_vec, par, "MPB");
        stat_Alg1 = update_stat(stat_Alg1, macrorep, PCS_path, FNR_path, ACC_path, N);

        [PCS_path, FNR_path, ACC_path, N] = Allocation_BM(T_vec, par, "MPB-PS");
        stat_Alg2 = update_stat(stat_Alg2, macrorep, PCS_path, FNR_path, ACC_path, N);

        [PCS_path, FNR_path, ACC_path, N] = Allocation_BM(T_vec, par, "MPB-ACC");
        stat_Alg3 = update_stat(stat_Alg3, macrorep, PCS_path, FNR_path, ACC_path, N);

        [PCS_path, FNR_path, ACC_path, N] = Allocation_BM(T_vec, par, "MPB-FNR");
        stat_Alg4 = update_stat(stat_Alg4, macrorep, PCS_path, FNR_path, ACC_path, N);

    end
    

    save(strcat(dir_name, '/Market_macrorep', num2str(AI), '.mat'), "stat_EA", "stat_COCBA", "stat_Alg1", "stat_Alg2", "stat_Alg3", "stat_Alg4", "T_vec");
end
